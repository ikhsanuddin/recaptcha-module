...
const API_ENTERPRISE_URL = 'https://www.google.com/recaptcha/enterprise.js'

class ReCaptcha {
  constructor ({ hideBadge, language, siteKey, version, size, type }) {
    if (!siteKey) {
  ...
  init () {
    ...
    if (this.version === 2) {
      script.setAttribute('src', `${this.type === 'enterprise' ? API_URL : API_ENTERPRISE_URL}` + '?hl=' + this.language)
    } else {
      script.setAttribute('src', `${this.type === 'enterprise' ? API_URL : API_ENTERPRISE_URL}` + '?render=' + this.siteKey)
    }
